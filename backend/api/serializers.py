...
class RecipeCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def save_ingredients(self, recipe, ingredients_data):
        ingredients_objs = [
            RecipeIngredient(
                recipe_id=recipe.id, ingredient_id=ing["id"], amount=ing["amount"]
            )
...
class FavoriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            recipe_id
            and Favorite.objects.filter(user=user, recipe_id=recipe_id).exists()
        ):
...
class ShoppingCartSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            recipe_id
            and ShoppingCart.objects.filter(user=user, recipe_id=recipe_id).exists()
        ):
